(function () {
  ...
  // Colors whose photos don't follow the "<color> <view>.png" naming pattern.
  var MOCKUP_OVERRIDES = {
    "stressed light": {
      front: "stressed light front.webp",
      back: "light washed shirt back.jfif"
    }
  };

  function mockupUrl(color, view) {
    var override = MOCKUP_OVERRIDES[color];
    var file = override && override[view] ? override[view] : color + " " + view + ".png";
    return "mockup/" + encodeURIComponent(file);
  }
  ...
  function addItem() {
    ...
    if (selectedProductIndex === CUSTOM_VALUE) {
      ...
      item = { sku: "", name: name, price: price, color: "", size: "", notes: notes, qty: qty };
    } else {
      var product = allProducts[selectedProductIndex];
      if (!product) {
